...
Video proof: capture a short clip (5–15 s) showing a successful pass-through run.
Place the file in the repository (example: `results/flyappy_run_01.mp4`) and link it in this report with markdown:

```
[Run video](results/flyappy_run_01.mp4)
```

To create a clip from a screen-recording or ROS visualization session, you can use `ffmpeg`:

```bash
ffmpeg -ss 00:00:05 -i screen_capture_full.mp4 -t 00:00:10 -c copy results/flyappy_run_01.mp4
```

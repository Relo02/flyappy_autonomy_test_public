...
**Repository files:**
- [flyappy_ros.py](flyappy_autonomy_code_py/src/flyappy_autonomy_code/flyappy_ros.py)
- [probabilistic_grid_map.py](flyappy_autonomy_code_py/src/flyappy_autonomy_code/probabilistic_grid_map.py)
...
2. [Dynamics & numerical integration](#2-dynamics--numerical-integration)
3. [Sampling & candidate rollouts](#3-sampling--candidate-rollouts)
4. [Cost function](#4-cost-function)
5. [Collision checking and probabilistic grid map](#5-collision-checking-and-probabilistic-grid-map)
6. [Heuristics: forward-block detection & perturbation](#6-heuristics-forward-block-detection--perturbation)
7. [Implementation notes and parameter knobs](#7-implementation-notes-and-parameter-knobs)
8. [Results (observed behavior)](#8-results-observed-behavior)
9. [Conclusions & trade-offs (precision vs speed)](#9-conclusions--trade-offs-precision-vs-speed)
10. [Next steps & improvements](#10-next-steps--improvements)
...
| Term | Description |
|------|-------------|
| Collision penalty | Large constant if any rollout point collides with an inflated obstacle (hard death) |
...
The Probabilistic Grid Map (PGM) stores occupancy belief on a local 2D grid and is updated from latest lidar scans, with scan points transformed in local coordinates using the current bird pose / velocity estimate.

**PGM responsibilities:**

1. **Scan fusion:** Fuse multiple scans over time to maintain memory of obstacles that temporarily leave the instantaneous scan (useful when passing behind obstacles or occlusions).
2. **Obstacle inflation:** Provide a set of occupied cells (positions) to the planner. These cells are inflated in the planner by an inflation radius (grid expansion) to account for the bird radius and motion uncertainty. In addition, the inflation helps to create more continuity in the obstacle representation, reducing the chance of skim collisions.
...
| Parameter | Value | Description |
|-----------|-------|-------------|
| `HORIZON` | 30 | Number of prediction steps |
| `NUM_PATHS` | 200 | Number of candidate rollouts |
| `SAFE_ACC_Y` / `MAX_ACC_Y` | 2.5 m/s² | Maximum vertical acceleration |
| `JERK_WEIGHT` | 20.0 | Weight for smoothness penalty |
| `EFFORT_WEIGHT` | 30.0 | Weight for control effort penalty |
| `Y_ERROR_WEIGHT` | 60.0 | Weight for gap attraction |
| `OBSTACLE_COST_WEIGHT` | 10.0 | Weight for obstacle clearance |
| `COLLISION_RADIUS` | 0.12 | Hard collision threshold |
| `TARGET_VX` | 0.3–1.0 m/s | Target forward speed (tunable) |
| `FWD_BIN_*` | — | Forward-block detection bins & offsets |
...
<p align="center">
  <img src="flyappy_autonomy_code_py/gif/mpc_planner.gif" alt="MPC Planner Demo" />
</p>
...
Higher obstacle penalties, larger horizons, and conservative sampling favor safe trajectories with large clearances but often result in slower average forward speed because the bird avoids risky but shorter paths.
...
### 2. Speed / Aggressiveness

Biasing `a_x` toward a larger `TARGET_VX`, lowering jerk/effort penalties, and reducing obstacle-cost scale let the bird commit to narrower gaps and travel faster, but increase collision risk especially if the grid map is noisy or the lidar FOV misses obstacles during the pass.

### Summary

Pushing for maximum speed requires careful risk-aware relaxation of obstacle penalties combined with robust gap detection and per-rollout clearance evaluation. The approach in `flyappy_ros.py` provides several levers (sampling bias, clearance cost, perturbation) to manage this trade-off.

> **Note:** The main drawback of this sampling-based MPC is that it can be sensitive to local minima when the gap is not partially visible in the lidar FOV.
...
Reinforcement learning (RL) can practically complement the sampling-based MPC in several ways.

**Possible pipeline implementation:**
...
| 3 | **Fine-tune with online RL** | Use PPO or SAC in simulation to improve performance and adapt to unseen layouts |
